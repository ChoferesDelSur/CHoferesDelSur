...
const fetchEntradas = async (idUnidad, periodo) => {
    ...
    if (periodo.tipo === 'semana') {
        url = route('reportes.cortesSRSemana', { idUnidad: idUnidad, semana: periodo.valor });
    } else if (periodo.tipo === 'mes' || typeof periodo === 'number') {
        url = route('reportes.cortesSRMes', { idUnidad: idUnidad, mes: periodo.valor });
    } else if (periodo.tipo === 'anio') {
        url = route('reportes.cortesSRAnio', { idUnidad: idUnidad, anio: periodo.valor });
    }
...
const generarArchivo = async (reporte, formato, idUnidad, periodoSeleccionado) => {
    ...
    try {
        ...
        if (reporte.titulo === 'Cortes Sin Regreso') {
            if (formato === 'pdf') {
...
const generarPDF = (tipo, periodoSeleccionado) => {
    ...
    doc.autoTable({
        ...
        body: bodyContent,
        startY: 24, // Ajusta el inicio de la tabla para que no se superponga con el título
        styles: {
            fontSize: 10, // Ajusta el tamaño de la fuente de la tabla si es necesario
...
const generarExcel = (tipo, periodoSeleccionado) => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Reporte_Cortes_SR');
