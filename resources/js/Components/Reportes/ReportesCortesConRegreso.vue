...
const fetchEntradas = async (idUnidad, periodo) => {
    ...
    if (periodo.tipo === 'semana') {
        url = route('reportes.cortesCRSemana', { idUnidad: idUnidad, semana: periodo.valor });
    } else if (periodo.tipo === 'mes' || typeof periodo === 'number') {
        url = route('reportes.cortesCRMes', { idUnidad: idUnidad, mes: periodo.valor });
    } else if (periodo.tipo === 'anio') {
        url = route('reportes.cortesCRAnio', { idUnidad: idUnidad, anio: periodo.valor });
    }
...
const generarArchivo = async (reporte, formato, idUnidad, periodoSeleccionado) => {
    ...
    try {
        ...
        if (reporte.titulo === 'Cortes Con Regreso') {
            if (formato === 'pdf') {
...
const generarExcel = (tipo, periodoSeleccionado) => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Reporte_Cortes_CR');
