...
const fetchEntradas = async (idUnidad, periodo) => {
    ...
    if (periodo.tipo === 'semana') {
        url = route('reportes.castigosSemana', { idUnidad: idUnidad, semana: periodo.valor });
    } else if (periodo.tipo === 'mes' || typeof periodo === 'number') {
        url = route('reportes.castigosMes', { idUnidad: idUnidad, mes: periodo.valor });
    } else if (periodo.tipo === 'anio') {
        url = route('reportes.castigosAnio', { idUnidad: idUnidad, anio: periodo.valor });
    }
...
const generarPDF = (tipo, periodoSeleccionado) => {
    ...
    const bodyContent = entradas.value.map(entry => {
        ...
        return [ruta, fecha, numeroUnidad, directivo, horaInicio, castigo, horaFin, observaciones, operador];
    });
    ...
    const docDefinition = {
        content: [
            { text: `Reporte de ${tipo} - Período: ${periodoTexto}`, style: 'header' },
            {
                table: {
                    headerRows: 1,
                    widths: ['*', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto', 'auto'], // Ajustar según el número de columnas
                    body: [
                        [
                            ...
                            { text: 'Castigo', style: 'tableHeader', alignment: 'center' },
...
const generarExcel = (tipo, periodoSeleccionado) => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Reporte_Castigos');
    // Guardar el archivo Excel
    XLSX.writeFile(workbook, nombreArchivo);
};

const reportes = [
    { titulo: 'Castigos', periodo: 'semana', periodoSeleccionado: 'semana' },
];
